...
class GoogleDocsUtilities extends GoogleUtilities {
   ...
   * @param domain
   */
  async createDoc(title, body, domain) {
    console.log(`${this._errorLogPrefix}Beginning createDoc`);
    try {
      let docObject = await this.getDoc(undefined, true);
      let response = await docObject.documents.create({title: title}, null);

      let googleDriveUtilities = new GoogleDriveUtilities(
          this._serviceAccount, this._privateKey, this._errorLogUrl, this._errorLogPrefix);

      // update permissions so can view as non service account user
      await googleDriveUtilities.updatePermissions(response.data.documentId, domain);
      console.log('here');

      console.log(`${this._errorLogPrefix}Finishing createDoc`);
      return `https://docs.google.com/document/d/${response.data.documentId}`;
    } catch (error) {
      console.log('The API returned an error: ' + error);
    }
  }
  ...
  //  * @param docId default is example doc id
  //  */
  // async writeToDocBody(docId) {
  //   // console.log(`${this._errorLogPrefix}Beginning writeToDocBody`);
  //   // try {
  //   //
  //   //   console.log(`${this._errorLogPrefix}Finishing writeToDocBody`);
  //   //   return body;
  //   // } catch (error) {
  //   //   console.log('The API returned an error: ' + error);
  //   // }
